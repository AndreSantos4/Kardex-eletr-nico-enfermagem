...
function obterValorDose(dose) {
  if (dose == null) return "—";
  if (typeof dose === "number" || typeof dose === "string") return dose;
...
function aplicarFiltros() {
  ...
  prescricoesFiltradas = todasPrescricoes.filter((p) => {
    return (p.medicamento?.nome ?? "").toLowerCase().includes(query);
  });
...
async function carregarPaciente() {
  try {
    ...
    if (btnKardex) {
      btnKardex.onclick = () => {
        window.location.href = `enfermeiroChefeKardexUtente?id=${patientId}`;
      };
    }
...
document.addEventListener("DOMContentLoaded", async () => {
  const nomeChefeEl = document.getElementById("nome-chefe");
  if (nomeChefeEl) {
    nomeChefeEl.textContent = sessionStorage.getItem("nomeEnfermeiro") ?? "—";
  }
