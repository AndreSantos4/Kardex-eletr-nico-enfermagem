...
function atualizarDosesEVia() {
  ...
  if (!selectMedicamento.value) {
    ...
    document.getElementById("verificacao-body").innerHTML =
      "Selecione um medicamento para verificação automática.";
    return;
  }
  ...
  const medicamento = medicamentosCache.find(
    (m) => m.id == selectMedicamento.value,
  );
...
function pacienteTemAlergia(medicamento) {
  ...
  for (const alergia of alergias) {
    ...
    if (
      ...
      textosSemelhantes(medicamento.principioAtivo, alergia)
    )
      return alergia;
...
async function carregarDados(id) {
  try {
    ...
    document.getElementById("cama").innerHTML = dados.processo.cama
      ? dados.processo.cama.id
      : "Sem cama";
    document.getElementById("diagnostico").innerHTML =
      dados.processo.diagnosticoPrincipal;
    document.getElementById("dias-internado").innerHTML = diasDesde(
      dados.processo.dataEntrada,
    );
...
function validarFormulario() {
  ...
  const indicacaoClin = document
    .getElementById("indicacao-clinica")
    .value.trim();
...
function validarDatas() {
  ...
  if (diffDias < 1) {
    mostrarNotificacao({
      titulo: "Erro",
      mensagem:
        "A data de fim deve ser pelo menos 1 dia após a data de início.",
      tipo: "erro",
    });
    return false;
  }
...
async function enviarPrescricao() {
  ...
  if (alergiaDetectada) {
    mostrarModalAlergia(medicamento, alergiaDetectada, () =>
...
async function submeterPrescricao(dados) {
  try {
    ...
    const resp = await fetch(
      `http://localhost:8080/api/processes/${processId}/prescriptions`,
      {
        method: "POST",
        headers: {
...
function inicializar() {
  ...
  document
    ...
    .addEventListener("change", atualizarIntervaloToma);
