...
async function recarregarPrescricoes() {
  ...
  ~console.log("Recarregando prescrições com estado:", estado);
...
async function carregarPaciente() {
  try {
    ...
    if (btnKardex) {
      btnKardex.onclick = () => {
        window.location.href = `enfermeiroKardexUtente?id=${patientId}`;
      };
    }
    ...
    if (btnPrescrever) {
      btnPrescrever.onclick = () => {
        window.location.href = `enfermeiroPrescreverMedicamento?id=${patientId}`;
      };
    }
...
document.addEventListener("DOMContentLoaded", async () => {
  document.getElementById("nome-enfermeiro").textContent =
